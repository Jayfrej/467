...
class MT5Handler:
    ...
    def get_supported_filling_mode(self, symbol):
        """Try filling modes and return first supported one"""
    ...
    def close_all_positions_by_type(self, symbol, position_type):
        ...
        for position in positions:
            if position.type == position_type:
                try:
                    filling_mode = self.get_supported_filling_mode(symbol)
                    if not filling_mode:
                        logger.error(f"No supported filling mode for {symbol}")
                        continue
